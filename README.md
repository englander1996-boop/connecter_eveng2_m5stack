...
4. グラスは数行しか出せないため、データを 7 つのページに分け、
...
| ページ | 表示内容 |
|--------|----------|
| **ACCEL** | 加速度（単位 g）。x / y / z の3軸と、その合成値 \|a\|。静止して水平に置くと z がほぼ 1.00（重力）、振ると値が暴れる |
| **GYRO** | 角速度（単位 deg/s）。x / y / z の3軸で、本体を回す速さ。静止中はほぼ 0。右端に IMU 温度も併記 |
| **TEMP** | IMU（センサーチップ）の温度。チップ自身の発熱を含むため、気温より数℃高めに出る点に注意 |
| **MIC** | 内蔵マイクの音量。0〜100% のレベルと `[####....]` のバー、dBFS 値。静かな部屋で 10% 前後、話しかけると跳ね上がる |
| **GPS** | 外付け GPS ユニットの状態と位置。ユニット探索中 → 衛星待ち（`sats` が捕捉数）→ 測位成功で緯度・経度・高度(m)・速度(km/h) に切り替わる。測位には空が見える場所が必要 |
| **POWER** | M5 のバッテリー残量（%）・電圧（mV）・充電中かどうか、稼働時間と RTC 時刻 |
| **TOUCH** | M5 の画面に触れている指の本数と座標、RTC 時刻。M5 の画面を触りながら見ると変わる |

各ページの最下行には共通で通信状態（`LINK ok  n=連番` または `connecting M5...`）が出る。
`n` が増え続けていれば M5 からデータが届いている。
